...
> **Beta Status**: v0.5.4, fully functional, actively developed. Report issues via [GitHub Issues](https://github.com/netwho/PacketCircle/issues).
...
**Version:**
- `v.0.5.4` (latest, default — just press Enter) — Wireshark-native UI refresh: pill toolbar, accent from host palette, restyled menus & legends, icon action bar, field-chip graph controls
- `v.0.4.7` — stable legacy release

**Feature set** (v.0.5.4 only):
...
> The default is v.0.5.4 Standard. Just press Enter twice to install with no prompts.
