...
The installer offers **v.0.5.4** (latest) and **v.0.4.7** (last v0.4.x). **Always choose v.0.5.4** unless you have a specific reason to stay on v.0.4.7.

| Version | Highlights |
|---|---|
| **v.0.5.4** (recommended) | Wireshark-native UI refresh: pill toolbar with host-palette accent, restyled context menus & graph legends, outline-icon action bar, field-chip graph controls (everything from v.0.5.3 included) |
...
# Install v.0.5.4 (latest):
cp installer/macos-universal/v.0.5.4/packetcircle.so \
...
- Recommends v.0.5.4 and warns if you select v.0.4.7
...
# (v.0.4.7 has ws40/ws42/ws44/ws46; v.0.5.4 has the same set)

# Example: Wireshark 4.6.x, v.0.5.4:
mkdir -p ~/.local/lib/wireshark/plugins/4.6/epan/
cp installer/linux-x86_64/v.0.5.4/packetcircle-ws46.so \
   ~/.local/lib/wireshark/plugins/4.6/epan/packetcircle.so

# Example: Wireshark 4.2.x, v.0.5.4:
mkdir -p ~/.local/lib/wireshark/plugins/4.2/epan/
cp installer/linux-x86_64/v.0.5.4/packetcircle-ws42.so \
   ~/.local/lib/wireshark/plugins/4.2/epan/packetcircle.so
```

### Supported Wireshark × Plugin Version Matrix (Linux)

| Wireshark | v.0.5.4 binary | v.0.4.7 binary |
|---|---|---|
...
- Recommends v.0.5.4; warns if you choose v.0.4.7
...
# Install v.0.5.4 (latest):
Copy-Item installer\windows-x86_64\v.0.5.4\packetcircle.dll `
